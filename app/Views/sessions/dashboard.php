    ...
    <div class="relative z-10">
        <div class="mb-12">
            <div class="flex items-center gap-3 mb-2">
                <div class="h-3 w-3 bg-green-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(34,197,94,0.6)]"></div>
                <h1 class="text-4xl font-extrabold font-headline tracking-tight text-on-surface">Sessions Actives</h1>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <?php foreach ($tables as $table): ?>
            <?php if (!empty($table['session_id'])): ?>
                <div class="group relative bg-surface-container-low p-8 rounded-xl border border-outline-variant/15 transition-all duration-500 hover:translate-y-[-4px]">
                    <div class="flex justify-between items-start mb-8">
                        <div>
                            <?php if (!empty($table['overtime'])): ?>
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-error text-on-error text-xs font-bold uppercase tracking-widest mb-4">
                                    Occupée ⚠️
                                </span>
                            <?php else: ?>
                    ...
                    <div class="space-y-6 mb-10">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-1">
                                <p class="text-xs text-on-surface-variant uppercase font-bold tracking-wider">Jeu en cours</p>
                                <p class="text-lg font-semibold flex items-center gap-2"><?= htmlspecialchars($table['game_name'] ?? 'Jeu') ?></p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-xs text-on-surface-variant uppercase font-bold tracking-wider">Client</p>
                        ...
                        <?php if (!empty($table['overtime'])): ?>
                            <div class="p-6 rounded-lg bg-error-container/20 border border-error-container/30 flex justify-between items-center">
                                <div>
                                    <p class="text-xs text-error uppercase font-bold tracking-wider">Temps dépassé</p>
                        ...
                        <?php else: ?>
                            <div class="grid grid-cols-3 gap-4 py-4 border-y border-outline-variant/15">
                                <div>
                                    <p class="text-xs text-on-surface-variant uppercase font-bold tracking-wider">Début</p>
                                    <p class="font-medium text-on-surface"><?= htmlspecialchars(date('H:i', strtotime($table['started_at'] ?? $table['start_time'] ?? 'now'))) ?></p>
                                </div>
                                <div>
                                    <p class="text-xs text-on-surface-variant uppercase font-bold tracking-wider">Prévu</p>
                                    <p class="font-medium text-on-surface"><?= $table['duration_hours'] ?? $table['duration'] ?? 1 ?>h</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-xs text-on-surface-variant uppercase font-bold tracking-wider">Écoulé</p>
            ...
            <?php else: ?>
                <div class="group relative bg-surface-container-low p-8 rounded-xl border border-dashed border-outline-variant/30 transition-all duration-500 hover:translate-y-[-4px] hover:border-primary/30">
